function initPartnersSlider() {
    ...
    sliders.forEach((slider) => {
        ...
        const pagination = slider.querySelector('[data-partners-pagination]');
        const slides = slider.querySelectorAll('.partners-slide');

        if (!track || !pagination || !slides.length) return;

        let currentPage = 0;
        let slidesPerPage = window.innerWidth <= 768 ? 3 : 6;
        let totalPages = 1;
        ...
        function buildPagination() {
            ...
            for (let i = 0; i < totalPages; i++) {
                ...
                dot.type = 'button';
                dot.className = 'partners-dot' + (i === currentPage ? ' active' : '');
        ...
        function updateDots() {
            const dots = pagination.querySelectorAll('.partners-dot');
        ...
        function updateSlider(withTransition = true) {
            slidesPerPage = window.innerWidth <= 768 ? 3 : 6;
            totalPages = Math.max(1, Math.ceil(slides.length / slidesPerPage));
            ...
            if (pagination.querySelectorAll('.partners-dot').length !== totalPages) {
                buildPagination();
            } else {
                updateDots();
            }
