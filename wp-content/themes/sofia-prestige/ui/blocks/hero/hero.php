...
$fallback_h2 = 'Агенство нерухомості та управління майном';
$fallback_h1 = '«СОФІЯ ПРЕСТИЖ»';
$fallback_h3 = 'Обирайте не адресу — а мрію!';

$first_text = $has_slides ? get_field('text', $slides[0]->ID) : [];
$initial_h2 = !empty($first_text['title1']) ? $first_text['title1'] : $fallback_h2;
$initial_h1 = !empty($first_text['title2']) ? $first_text['title2'] : $fallback_h1;
$initial_h3 = !empty($first_text['title3']) ? $first_text['title3'] : $fallback_h3;

$telegram_option = trim((string) get_option('sofia_contact_telegram', ''));
$phone_option = trim((string) get_option('sofia_contact_phone', ''));

$phone_label = $phone_option !== '' ? $phone_option : '+38 (068) 517-91-19';
...
$phone_href = $phone_option !== '' ? 'tel:' . preg_replace('/[^\\d+]/', '', $phone_option) : '#';
?>

<section class="hero">
    <div class="backgroundContainer">
        <?php if ($has_slides) : ?>
            <?php foreach ($slides as $slide_post) :
                $text = get_field('text', $slide_post->ID) ?: [];
                $img = get_field('slide', $slide_post->ID);

                $h2 = $text['title1'] ?? $fallback_h2;
                $h1 = $text['title2'] ?? $fallback_h1;
                $h3 = $text['title3'] ?? $fallback_h3;

                $img_url = $img['url'] ?? get_template_directory_uri() . '/assets/img/hero.webp';
                $img_alt = $img['alt'] ?? get_the_title($slide_post);
            ?>
                <div class="slide" data-h2="<?php echo esc_attr($h2); ?>" data-h1="<?php echo esc_attr($h1); ?>" data-h3="<?php echo esc_attr($h3); ?>">
                    <img src="<?php echo esc_url($img_url); ?>" alt="<?php echo esc_attr($img_alt); ?>">
                </div>
            <?php endforeach; ?>
        <?php else : ?>
            <div class="slide" data-h2="<?php echo esc_attr($fallback_h2); ?>" data-h1="<?php echo esc_attr($fallback_h1); ?>" data-h3="<?php echo esc_attr($fallback_h3); ?>">
                <img src="<?php echo get_template_directory_uri(); ?>/assets/img/hero.webp" alt="Hero slide">
            </div>
        <?php endif; ?>
    </div>
    ...
    <div class="container">
        <div class="content">
            <h2><?php echo esc_html($initial_h2); ?></h2>
            <h1><?php echo esc_html($initial_h1); ?></h1>
            <h3><?php echo esc_html($initial_h3); ?></h3>
        ...
        <div class="sliderIndicator">
            <button class="sliderButton active"></button>
        </div>
    </div>
</section>
